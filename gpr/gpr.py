...
def estimate_metals_gpr(data, regr_file, ss_cuts_file, regr_type):
    '''
        ...
        ss_cuts_file: (str)
            The file name single star cuts used to indentify
            possible unresolved binaries in a dataset
        ...
        -------
        M_H: (numpy array)
            Estimate metallicities from Gaussian Process Regressor.
            All entries with 99.0 either have estimated
            values outside the bounds or did not pass single star cuts.
        M_H_std: (numpy array)
            Estimate confidence on metallicities from Gaussian Process
            Regressor. All entries with 99.0 either have estimated values
            outside the bounds or did not pass single star cuts.
    '''
    regr = joblib.load(regr_file)
    if regr_type == 'M':
        # create the colors and absolute magntiudes needed
        # for the regression and single star cuts
        ...
        # mean and standard deviation of the training subset used
        # to normalzie the data
        ...
        # create the colors and absolute magntiudes needed for the regression
        # and single star cuts
